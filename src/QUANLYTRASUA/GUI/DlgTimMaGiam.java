        ...
        if (row > -1) {
            try {
                if (tblMaGiam.getValueAt(row, 6).equals("Không hiệu lực")) {
                    new Dialog("Mã này đã hết hiệu lực!", Dialog.ERROR_DIALOG);
                    loadDataLenTable();
                    return;
                }
                ...
                if(dieuKien > tongTien) {
                    new Dialog("Không đủ điều kiện áp dụng mã giảm này!", Dialog.ERROR_DIALOG);
                    return;
                }
